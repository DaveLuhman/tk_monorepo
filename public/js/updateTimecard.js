...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const validation = {
    ...
    status: function () {
      ...
      if (validation.empName && validation.empEmail && validation.jobNames) {
        ...
        return 'The job name field cannot be blank. Delete the row or add a name.' // jobName validation failure route
  ...
  function validateJobName() {
    ...
    let result = Array.from(jobNameInputs).some(
      (input) => input.value.trim() !== ''
    )
    validation.jobNames = Array.from(jobNameInputs).every(
      (input) => input.value.trim() !== ''
    )
  }
